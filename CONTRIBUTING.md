...
(`ruff`, `bandit`), catching issues before you even push. GitHub Actions
CI is currently removed (see `docs/CI_CD.md`) — these hooks are the main
line of defense in the meantime, so please keep them installed.
...
- **Linting/formatting**: [`ruff`](https://docs.astral.sh/ruff/) — config
  ...
  three.
- **Security**: [`bandit`](https://bandit.readthedocs.io/) runs via
  pre-commit.
